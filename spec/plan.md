...
- [x] Step 25.9: **Android** — "Find Users" tab replaced by **Add Friends**: "MY INVITE CODE" card (fetched lazily via `POST invite-code`; copy + share-sheet with explainer text, expiry shown), "Have a friend's code?" field + Add Friend by Code (GlassButton, per-action loading), "Or add by email" field + Send Friend Request (client-side complete-email check; shows the server's anti-enumeration generic message in a green success card). `sendFriendRequest` retyped to `FriendRequestResultDto` (the 25.7 breaking change); partial user search removed from repository/ViewModel. `DashboardViewModel` fires `claim-invites` once per session start (online only, fire-and-forget); `MyFirebaseMessagingService` handles `FRIEND_ADDED_VIA_CODE`. Verified live with two accounts on one emulator: Aariya's code CR549B fetched from the deployed API, signed into the mock test user, redeemed the code → "Code correct! You are now friends with Aariya Ojha (a***@g***.com)" with masked email, friendship instantly visible in BOTH accounts' My Friends lists. Test friendship cleaned from Mongo afterwards.
- **COMMIT (Android)**: "feat: invite-code friend discovery UI - Add Friends tab"
- **⚠ Config note**: `appsettings.Development.json` holds a real SMTP password in plaintext and is deliberately left uncommitted — move it to an env var / user-secrets and rotate the credential.
...
- [x] Step 41.4: **Game Action Panel (`GameActionPanel.kt`)** — Displays header and action tiles: `Settle` (wallet), `Scores` (leaderboard), `Share` (social card export), `Theme` (palette picker), and `Delete` (danger set delete with confirmation dialog for host). The disabled `Host (Soon)` placeholder has been hidden per UI refinements. Tapping any action executes the command and auto-collapses the panel.
