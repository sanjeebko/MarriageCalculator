...
public class MarriageGameSetService(IMarriageGameSetRepository gameSetRepository) : IMarriageGameSetService
{
    ...
    public async Task<IEnumerable<MarriageGameSetDto>> GetAllGameSetsAsync(int gameSettingsId)
    {
        var gameSets = await _gameSetRepository.GetByGameSettingsIdAsync(gameSettingsId);
